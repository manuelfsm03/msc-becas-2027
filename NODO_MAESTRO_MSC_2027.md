# NODO MAESTRO — Búsqueda MSc Economics 2027 (Manuel)

> **Actualizado: 13-sep-2026 (HSG St. Gallen entra al foco activo por decisión de Manuel: el foco pasa a ser Banco de España + Inova/UK + HSG. Antes: 9-sep, dossier Suiza y lectura completa de las bases oficiales del BdE; 16-ago, McCall MacBain/McGill desistido).** Este documento SUPERSEDE a `CONTEXTO_MSC_traspaso.md` y a `briefing_manu_masters_v2.md`. Es el nodo de sincronización entre el trabajo en Cowork local y el proyecto de chat en Claude.ai. La fuente de datos viva es `masters_consolidado_GRE_2027.xlsx` (carpeta local MSc); este nodo resume su estado y decisiones tomadas.

## 1. Perfil

Manuel, argentino + **ciudadanía española** (pasaporte español = fee status EU/EEA). Lic. en Economía UBA, cum laude (7.82/10). Trabaja en Bunge + IIEP (UBA-CONICET) + docencia FCE-UBA. Python, econometría aplicada, series de tiempo, inferencia causal. Inglés: Cambridge C1 certificado (IELTS pendiente, rendir antes de ene-2027). Inicio objetivo: **septiembre 2027**. Abierto a PhD posterior.

Dos tracks de interés: **(T1) Economics & Finance / Política Monetaria** (cuantitativo) y **(T2) Development Economics / Inferencia Causal / Econometría**.

**Preferencia de duración (jul-2026):** programas de 1 año, con bonus leve editable en el ranking (no penalización dura a los de 2 años).

## 2. Decisiones estratégicas vigentes

- **Palanca clave:** pasaporte español → tuition EU: gratis (Alemania, nórdicos) o casi (Francia ~€359, Bélgica ~€1.2k, Austria ~€726, NL €2.694). En el continente muchas veces NO necesita beca, solo costo de vida (~€11-16k/año). UK sí exige beca full o quasi-full. **⚠️ EXCEPCIÓN verificada 17-jul-2026: IRLANDA — el fee status EU exige además residencia en la UE 3 de los últimos 5 años; residiendo en Argentina paga fees non-EU pese al pasaporte (mitigable con GOI-IES).**
- **GRE objetivo (verificado jul-2026): Q ≥165 / V ≥158 / AWA ≥4.0 pasa todos los filtros.** Pisos duros: Tinbergen Q160, LSE Q~164, SSE Q155. Ningún programa exige GMAT en exclusiva. Groningen MSc regular ya no pide GRE (Research Master sí). **McGill MA: GRE REQUERIDO** para grados no canadienses (GMAT no sustituye).
- **Restricción Norteamérica RELAJADA (jul-2026):** originalmente descartada por costo; ahora se consideran oportunidades **full-funding** caso por caso (disparador: McCall MacBain/McGill). No se agrega Norteamérica en general, solo becas realmente completas.
- **GREAT España: DESCARTADA** (exige residencia permanente en España). Eliminada de todos los archivos jul-2026.
- Trabaja con **Inova Education** para UK/NL (máx. 5 aplicaciones UK). Asesora: Fabiola Delfín. **To-do list de documentos y form: `INOVA_todo_documentos.md`** (17-jul-2026; basada en el mail de Fabiola del 14-may + form real de inovaeducation.com/apply-now). Form llenable desde agosto; documentos clave a iniciar YA: analítico FCE-UBA + traducción pública (sirve para Inova + McCall + visa).
- **HSG St. Gallen entra al foco activo (13-sep-2026, decisión de Manuel).** El foco pasa a ser Banco de España + Inova/UK + HSG. Sustento: es el mejor fit de método del mapa — el SEW-HSG tiene bajo un mismo techo a Lechner (causal machine learning para política pública) y a Koeniger (transmisión monetaria y fiscal identificada con datos transaccionales), ver `MATCH_grupos_profes_por_uni.md` → BLOQUE SUIZA —, el MEcon no exige GRE, y HSG Best Talents es la única beca suiza que entra por el pasaporte español (`SUIZA_dossier.md` §3.3). Consecuencia operativa: aplicar a MEcon desde el 1-oct-2026, porque Best Talents (portal nov-2026, rolling) exige estar admitido. **Queda abierto** el reparto de cartas entre MEcon y MiQE/F.

## 2 bis. ⛔ Filtro transversal: la experiencia laboral se cuenta DESPUÉS del título (2-ago-2026)

Este es el hallazgo que más opciones cierra de golpe, y estaba mal en el proyecto desde el principio. Los archivos decían *">2 años de experiencia laboral profesional (relevante para Chevening, DAAD, World Bank)"*, contando **experiencia total**. La regla real, al menos en Chevening, cuenta **solo desde la graduación**:

> *"Have at least two years' work experience, **acquired after completing your undergraduate degree**, equivalent to 2,800 hours."* — [criterios de elegibilidad oficiales](https://www.chevening.org/resource-hub/guidance/eligibility/), verificado 2-ago-2026

Y hay una segunda cláusula independiente que ata lo mismo a la fecha de egreso: *"You must have finished your undergraduate studies at least two years before our application deadline."*

**Consecuencias:**

- **Chevening ❌ DESCARTADA** para el ciclo 2027-28. No llega a los 2 años post-título. Decisión de Manuel del 2-ago-2026, verificada contra la fuente. No es una cuestión de competitividad: es un filtro duro de elegibilidad. Con esto quedan sin efecto todas las notas de incompatibilidad Chevening ↔ GREAT-Glasgow / Southampton Presidential / Banco de España.
- **DAAD EPOS ⚠️ casi con certeza cae por lo mismo.** El GANTT ya lo marcaba como "exige ≥2 años de experiencia POST-título: verificar contra tu fecha de egreso". Ahora sabemos que ese tipo de cláusula sí se aplica literal. Verificar antes de invertir tiempo.
- **World Bank JJ/WBGSP ⚠️** — misma familia de requisito, verificar.
- **El Banco de España sube de importancia.** Sus bases **no piden experiencia laboral**: solo nacionalidad española + título de grado. La familia de becas que se cierra por experiencia post-título es justamente la que se apoyaba en el pasaporte argentino; la vía española no toca ese filtro.

**Regla para el futuro:** ante cualquier beca que pida "X años de experiencia", chequear si cuenta desde la graduación antes de darla por cubierta. Y no propagar "tiene 2+ años de experiencia" como hecho genérico del perfil: depende de cómo cuente cada convocatoria.

## 2 ter. Regla de método para buscar becas (2-ago-2026)

Antes de salir a buscar más becas, **diffear las clases de fuente ya usadas**, no los ítems. La 6ª tanda había concluido "la barrida está saturada, no hay agujeros relevantes" — y era cierto *dentro de las tres clases que se venían mirando* (financiadores primarios, lado argentino de las becas de Estado, catálogos y listas). Las clases nunca tocadas eran el lado **español** del Estado, los institutos y bancos centrales, y los tableros de empleo. De ahí salió el Banco de España, que es la mejor beca del mapa.

Clases que siguen sin barrer: `capacity_database` (CORDIS, GEPRIS, ERC) y `bibliographic` (OpenAlex) — ambas solo valen la pena si la ruta PhD entra en juego.

## 3. Archivo maestro: `masters_consolidado_GRE_2027.xlsx`

9 hojas: Resumen GRE · **Ranking** (score ponderado, pesos editables en amarillo: Fit 25 / Costo neto 25 / Financiabilidad 20 / Prestigio 20 / Admisibilidad 10, + bonus 0.25 a ≤1 año) · UK Deep-Dive · Todos los programas (**109** programas) · Filtro GRE-GMAT · Becas (categorías A universidad / B fundaciones / C gobiernos) · Timeline · Perfil y estrategia · Pendientes.

Top del ranking con pesos default: Bonn / Mannheim / PSE-PPD (4.80), Groningen / Radboud / Maastricht (4.75), Goethe MMF / Aarhus / PSE-APE / Tilburg / KU Leuven (4.70). UK queda ~3.4-3.7 porque el score asume funding incierto; si cae Chevening/Warwick Sudamérica/CEMFI-Carolina, ese programa salta al tope de facto.

**Top picks continentales:** Goethe Frankfurt MMF (T1, gratis, cerca del ECB) y PSE Master PPD (T2, ~€359). Rutas full-funded: Erasmus Mundus QEM/EPOG+ y CEU Viena (Hantos para argentinos).

## 4. UK shortlist FINAL (ranking propio de Manuel, jul-2026)

👑 Tier 1: **Bristol, Warwick, Glasgow**. Tier 2 en orden: 1-Southampton (CONDICIONAL: Presidential fuera de línea entre ciclos, chequear ~oct-2026; elegibilidad por RESIDENCIA, Argentina listada; no permite beca externa), 2-Birmingham (BBS hasta £25k, verificado aplica), 3-Essex (piso más barato + 25% cuasi-auto), 4-Bath (£10k prob. alta, con tope), 5-Exeter (reserva). Sussex descartada salvo T2/PhD. Slots Inova (5): corona + [Southampton si se confirma + Birmingham], si no [Birmingham + Essex/Bath].

Correcciones verificadas: Bristol Think Big £26k SÍ aplica a MSc Econ; Glasgow becas NO apilables; Warwick regional = pool África+Sudamérica combinado.

## 5. NUEVO (17-jul-2026): Norteamérica y becas investigadas

### McCall MacBain / McGill — ❌ DESISTIDO (16-ago-2026)

Manuel desistió de McCall MacBain y McGill MA Economics: el endorsement UBA obligatorio (la UBA no es universidad participante) no salió a tiempo — sin él, no se puede aplicar a la beca. Con esto se cae también el plan B asociado (PBEEE Québec, apilable con McGill) y el pendiente #3 de contactar supervisores McGill. El foco pasa a **Banco de España + Inova/UK** en paralelo.

Queda como registro histórico (checklist, cronograma, supervisores, templates) en `MCCALL_MCGILL_plan_ataque.md` — no seguir usándolo para planificar.

<details>
<summary>Detalle original (17-jul-2026) — supersedido</summary>

Checklist completo, cronograma semanal, shortlist de supervisores McGill y template de mail en **`MCCALL_MCGILL_plan_ataque.md`**. Puntos nuevos verificados en la web oficial:
- **⚠️ Endorsement UBA obligatorio:** la UBA NO es universidad participante (LatAm: solo USP, UFMG, UNAM, Tec, Los Andes, Nal. Colombia, PUCP, U. Lima, UWI) → el aplicante de universidad no participante debe conseguir endorsement de su universidad, sin importar el año de graduación. Iniciar trámite en FCE-UBA YA.
- **2 referencias** (1 académica + 1 comunitaria, no familiares/amigos/compañeros) completan un form online **antes del 19-ago**; también hay que declarar **verificadores** de actividades.
- Componentes: datos personales · hasta 5 programas McGill · actividades (4 comunitarias, 3 obras, 5 trabajos) · 3 short answers (105 caract.) · 4 essays (comunidad+programa 225+225 / liderazgo 275 / contribución 250 / course correction 250) · resumé 1 pág. PDF · transcripts con traducción no certificada · GPA convertido a escala 4.0.
- 10 becas full intl + ~100 entrance awards; semifinales oct-dic 2026, finales en Montreal mar-2027 (viaje pagado).

### Supervisores McGill (scouting 17-jul-2026)

Prioridad máxima (los tres verificados activos, con **conexión UBA los dos primeros... y el tercero LATAM**): **Santiago Cámara** (BA UBA 2013; macro internacional, spillovers Fed/ECB a emergentes — T1 perfecto), **Nicolás Ajzenman** (BA UBA; desarrollo/behavioral/political economy, J-PAL — T2 perfecto), **Fernando Saltiel** (labor/educación en desarrollo, ex IADB/Brookings — T2). Segunda línea: Ruge-Murcia, Poschke, Castro (T1); Amodio, Chemin, Laszlo (T2); Dufour/Gonçalves/Chaudhuri (econometría). Contactar en tandas desde fines de agosto (post McCall MacBain).

### McGill MA Economics + McCall MacBain — ✅ AGREGADO, prioridad alta
- **MA Economics (Thesis)**: 1 año (3 terms, sep-ago), PhD-prep explícito (micro/macro/econometría, se eligen 2 de 3 campos). Tuition intl ~CAD 25.7k + vida ~CAD 18k. **GRE REQUERIDO** para grados no canadienses — verificado en la página del departamento (how-to-apply, nov-2025): GMAT no sustituye; códigos GRE 0935 (McGill) / 1801 (Economics). **Inglés (verificado GPS jun-2026): SOLO IELTS Academic ≥6.5 (presencial, envío electrónico por el centro) o TOEFL iBT ≥86 (mín. 20/sección) — NO aceptan Cambridge C1, Duolingo ni PTE** → el C1 de Manuel no sirve acá; rendir IELTS antes de dic-2026 por demoras reportadas en el envío. **Deadline programa: 15-ene-2027 (intl).** Non-thesis suspendido desde 2024 → la vía es thesis, y **exige supervisor de McGill Economics confirmado AL MOMENTO de aplicar** (contactar profesores meses antes del deadline). Requisitos de mate: 2 cursos de cálculo + álgebra matricial + estadística anual (perfil UBA cubre).
- **McCall MacBain Scholarship**: FULL (tuition + fees todo el programa + CAD 2.300/mes + relocation + verano). Sin restricción de nacionalidad; elegible si el grado se obtuvo hace <5 años (o ≤30 años). ~30 becas intl/año; finalistas no ganadores reciben entrance awards CAD 5-20k. Selección tipo Rhodes (liderazgo + servicio + académico). **⏰ Deadline internacional: 19-AGO-2026** (entrevistas nov-2026 y mar-2027). Se aplica a la beca Y al programa por separado.

</details>

### DAAD Helmut Schmidt (PPGG) — ⚠️ elegible pero fit parcial
Argentina elegible; full funding (~€992/mes + seguro + viaje + 6 meses de alemán). **PERO solo másters de Public Policy/Good Governance** (Development & Governance Duisburg-Essen, MPP Erfurt, Development Studies/Governance Passau, Social Protection H-BRS, etc.) — **no cubre MSc Economics**. Solo tiene sentido si Manuel quisiera pivotar al track policy/desarrollo institucional. Ventana: 1-jun a 31-jul-2026 (intake 2027), aplicación directa a las unis (hasta 2). Resultado dic-2026/ene-2027.

### Ireland Fellows — Latin America (Cecilia Grierson) — ❌ para economía
Full funding (fees + vuelos + alojamiento + €700/mes), Argentina elegible como "resident national", Stage 1: 29-jun a 26-jul-2026. **Verificado en la guidance note oficial: para argentinos el award Grierson es SOLO STEMM** — secciones B (Engineering), E (Salud), I (Data/IT), K (Food Science) del directorio. **MSc Economics (sección J) y Development (sección C) NO son elegibles para argentinos.** Además exige 3 años de experiencia laboral relevante. Solo serviría pivotando a MSc Data Analytics/Data Science (sección I). Descartada salvo cambio de objetivo.

## 6. Becas — mapa por categoría (hoja Becas del Excel)

- **A (universidad):** Warwick Sudamérica (2×100% tuition), Bristol Think Big £26k, Glasgow ASBS ×3 + World Changers, Edinburgh (varias, LATAM ✅), Birmingham BBS £25k, Bath, Essex 25%, Southampton Presidential (condicional oct-2026).
- **B (fundaciones):** Ramón Areces ✅ (fit perfecto vía ESP, ~feb-2027), Rafael del Pino ✅ (~23-feb-2027), **la Caixa ❌ (RESUELTO 17-jul-2026: bases oficiales exigen grado de universidad ESPAÑOLA; título UBA descarta pese a nacionalidad)**, **Carolina ✅ (RESUELTO 17-jul-2026: FAQ oficial — con doble nacionalidad se postula bajo la nacionalidad ARGENTINA; requisito no residir en España; clave CEMFI/BSE)**, **McCall MacBain ✅ (19-ago-2026)**, Mutua Madrileña ❌, Amancio Ortega ❌.
- **C (gobiernos):** **Chevening ❌ DESCARTADA (2-ago-2026, ver §2 bis)**, DAAD EPOS ⚠️ (~ago-2026, en riesgo por la misma regla), **DAAD Helmut Schmidt ⚠️ (31-jul-2026, solo PPGG)**, Erasmus Mundus QEM/EPOG+ ✅, World Bank JJ/WBGSP ✅, Eiffel ⚠️, Swiss Excellence ⚠️ (solo PhD/postdoc), Fulbright ❌, **Ireland Fellows ❌ (solo STEMM para ARG)**, GREAT España ❌ (eliminada).
- **Barrida Campus Argentina Global (17-jul-2026):** **IYT Italia ⚠️** (Economics elegible, waiver + estipendio, conv. abr-may-2027; ❗límite de edad 25 — verificar) · **Stipendium Hungaricum 🔍 agregada** (full masters, ARG por bilateral, conv. nov-ene; respaldo) · **Ernst Mach/OeAD ❌** (solo estadías de investigación 1-9 meses, no másters) · **Swedish Institute ❌** (Argentina fuera de la lista de 34 países; Suecia igual es gratis vía EU) · **BEC.AR ❌** (solo cursos cortos en áreas estratégicas).
- **Cierre becas pendientes (17-jul-2026, 2ª tanda):** ARES ❌ y VLIR-UOS ❌ (Argentina fuera de las listas de países) · Aga Khan ❌ (13 países, ARG fuera; 50% préstamo) · fundaciones alemanas ⚠️ (KAS €992/mes pero exige alemán B2 + máster de 4 semestres; solo si aprende alemán) · **Rotary Global Grant ⚠️ VIABLE** (US$30.600 matrícula+vida+pasaje; "economic and community development" encaja con T2; requiere patrocinio de club Rotary local + club anfitrión — contactar distrito BA ~sep-2026).

### Barrida Norteamérica full-funding (17-jul-2026)

- **Fulbright ARG reevaluada** (política NA relajada): Programa Regular Máster/Doctorado — pasaje + estipendio hasta US$3.231/mes + asistencia para waivers de matrícula; economía y desarrollo es área prioritaria; requisitos cumplibles (promedio ≥7 ✓, ~2 años experiencia, inglés). **❗El ciclo 2027-28 cerró el 25-mar-2026** → solo sirve para intake 2028-29 (convocatoria ~ene-mar-2027). Pendiente: decidir si 2028 entra en juego.
- **Knight-Hennessy (Stanford) ⚠️**: full tipo McCall, cualquier nacionalidad, título ≤7 años, deadline 6-oct-2026. Pero Stanford no tiene MSc/MA Economics → solo tiene sentido vía PhD directo o MPP/MS.
- **PBEEE Québec ✅ (la joya de la barrida): CAD 20.000/año (hasta 40k) para maestría, apilable con McGill.** Requiere nominación de la universidad (pedir a McGill, deadlines internos ~oct-nov-2026; FRQ 2-dic-2026). McCall MacBain (plan A) + PBEEE/TA-ships (plan B) cubren bien el riesgo de McGill.
- **Western MA Economics ✅ agregada**: paquetes de funding (tuition + fees + TA para vida) decididos automáticamente con la admisión, 1 año, PhD-prep. Candidata a "Todos los programas"/Ranking como plan B de NA. Toronto/UBC: solo funding parcial por mérito → no cumplen la política full.
- Descartes NA: Vanier/Trillium (solo PhD), OEA (parcial).

### Lista nueva de 24 becas — verificada (17-jul-2026, 3ª tanda)

Informe completo con los 24 veredictos: **`BECAS_veredictos_lista_24.md`**. Lo nuevo accionable:

- ⛔ **CORREGIDO 25-sep-2026: DAAD Study Scholarship ❌. Argentina no figura en la lista cerrada de países de residencia de la ficha oficial; lo de abajo es el análisis viejo, que estaba mal.** ~~DAAD Study Scholarship ✅ ⭐~~ **DAAD Study Scholarship (másters todas las disciplinas) — elegibilidad UE confirmada sin restricción (25-ago-2026).** ~€992/mes × 10-24 meses para CUALQUIER máster en Alemania — sobre matrícula gratis convierte Bonn/Mannheim/Goethe en paquete full. Deadline ~31-oct-2026 ⚠️ (fecha exacta no confirmada esta tanda, confirmar en portal DAAD ARG). **Dossier a fondo: `DAAD_STUDY_SCHOLARSHIP_dossier.md`** — resuelve la pregunta de si el pasaporte español (patrón de exclusión visto en la Caixa/GREAT/Ireland/Hungría) afecta esta beca: **no afecta.** La ficha oficial del programa no tiene lista de países elegibles ni cláusula de exclusión por nacionalidad (a diferencia de EPOS/Helmut Schmidt, que sí son development-focused). La regla de doble nacionalidad de DAAD es por país de origen/residencia, no por pasaporte adicional — y el ejemplo oficial que da DAAD para explicar la regla es, cambiando Italia por España, el caso exacto de Manuel: aplica por Argentina, vía DAAD Information Centre Buenos Aires. El mismo dossier suma 5 candidatos nuevos de MSc Economics en Alemania más allá de Bonn/Mannheim/Goethe MMF: **Heidelberg (AWI, versátil T1+T2 en un solo programa vía tracks) y HU Berlin (MSc Economics puro, T1 vía Monetary Economics, sin GRE)** como los mejores hallazgos; también Kiel (ligado al IfW ya conocido por el ASP), Cologne (Economic Research, feeder de PhD) y, con reservas a depurar, Konstanz y Tübingen. Frankfurt School queda descartada del radar "gratis por pasaporte ESP": es privada (~€35-42k) y exige GMAT/GRE sin waiver.
- **Master Mind (Flandes) ✅ ⭐**: €10.225/año + matrícula ~€137, todas las nacionalidades; se aplica vía la institución → KU Leuven (ya 4.70 en el ranking) queda a costo ~cero. Feb-abr-2027.
- **Clarendon (Oxford) ✅ + Gates Cambridge ✅**: full, todas las nacionalidades; Clarendon es automática con la app a Oxford y Gates es un tilde en la app a Cambridge → sobre la MISMA aplicación de MPhil Econ hay 3 vías full (Clarendon + Weidenfeld + college funds / Gates). Revisar al alza la financiabilidad de Oxbridge en el ranking UK.
- **GOI-IES Irlanda ✅**: €10.000 + waiver total (60 becas), conv. ~ene-2027 con oferta en mano. **Hallazgo asociado — CORRECCIÓN al perfil: en Irlanda el fee status EU exige residencia UE 3 de los últimos 5 años → viviendo en Argentina, Manuel paga fees NON-EU pese al pasaporte español.** Irlanda no era "barata vía EU"; GOI-IES es la compensación.
- **ETH ESOP ⚠️**: CHF 12k/semestre + waiver, ventana única 1-30 nov-2026; ETH no tiene MSc Econ puro (fit = MTEC o Quantitative Finance).

#### 🇨🇭 SUIZA — barrida a fondo (9-sep-2026, 8ª tanda). Dossier: `SUIZA_dossier.md`

Suiza estaba cubierta sólo por el lado de las becas. Barrida completa del catálogo oficial (87 programas → 9 candidatos reales) con costos, becas y fit verificados.

- ⛔ **CORRECCIÓN AL PERFIL — segunda excepción a la palanca del pasaporte, después de Irlanda y por un mecanismo distinto: en Suiza el pasaporte español NO reduce la matrícula.** Suiza no diferencia por ciudadanía sino por **origen del título de secundaria** ("non-Swiss educational qualifications"): con secundario argentino se paga tarifa extranjera siempre. La matrícula es barata igual — CHF 870 (Ginebra) · 1.000 (Lausana) · 1.640 (UZH) · 1.700 (Basilea) · 5.100 (Berna) · 6.634 (HSG) · 8.000 (IHEID/USI) al año.
- **Pero el pasaporte sí sirve para tres cosas:** (1) **HSG Best Talents** ✅ ⭐ — la única beca de máster suiza restringida a nacionales **UE/EFTA** o permiso C: matrícula completa + aporte a costo de vida, 39 plazas → 45, **portal reabre nov-2026 para la cohorte 2027**, exige estar ya admitido a un máster de HSG (⚠️ monto del aporte de vida sin publicar); (2) permiso B UE/EFTA sin visado y **sin el tope de 15 h/semana ni la espera de 6 meses** que atan a los no-UE → el trabajo de asistente es línea de financiación real; (3) deadlines más holgados (UNIL: 30-abr, contra 28-feb para quien necesita visado — mismo mecanismo que Konstanz).
- **El costo de vida es el único problema real: CHF 18-23k/año en St. Gallen, Berna y Basilea; CHF 24-30k en Zúrich y Ginebra**, más KVG obligatorio CHF 280-380/mes ⚠️ (posible exención, ~CHF 4.000/año en juego).
- **Suiza se comporta como UK, no como Alemania: exige beca full.** No hay DAAD suizo ni Master Mind y Erasmus+ no la cubre. **Con el BdE queda esencialmente autofinanciable en St. Gallen / Berna / Basilea** y corta CHF 8-14k/año en Zúrich y Ginebra. Sin BdE, la única vía propia es Best Talents.
- **Programas, por fit con la espina:** **HSG MEcon** ★★★★ (SEW-HSG de Michael Lechner: causal machine learning para evaluación de política pública — el mejor fit de método de todo el mapa continental; **GRE/GMAT OPCIONAL**, si no se presenta el promedio pondera doble; 1-oct a 31-mar; la ciudad más barata) · **UZH MA Economics** ★★★★ (RePEc #1 de Suiza, Pomeranz + Yanagizawa-Drott, ya listada en la ruta predoc; pero Zúrich carísima, sin beca de máster, ⚠️ deadline sin verificar) · **UNIL MSc Economics** ★★★★ (único programa que cubre T1 y T2 explícitamente: menciones en Macroeconomic Policy, Development Economics, Public Economics, Quantitative Economics, Data Science; ⚠️ riesgo de *bridging programme* para título UBA, sin resolver) · **MIME Basilea+Berna** ★★★ (T1 puro, BIS + SNB + Gerzensee; ir por Basilea: CHF 850/sem vs 2.550 de Berna; ⚠️ exige 5,0/6 o GRE) · **HSG MiQE/F** ★★★ (GRE obligatorio; permite designar MEcon como alternativa si se aplica antes del 31-ene) · **IHEID Ginebra** ★★★ (única beca propia decente de Suiza: hasta CHF 20k por dos años, mérito+necesidad) · UNIGE ★★ · UniBE ★★ · UniNE ★. USI y Unilu descartadas.
- ESKAS ❌ **reconfirmada** (2027-28 exige máster completo al 31-jul-2027: es research/PhD). ETH ESOP y EPFL Excellence ⚠️ sin cambios (siguen sin MSc Econ).
- **Eiffel re-verificada ✅**: límite máster ≤29 años confirmado (el dato del Excel era correcto); la postula la universidad.
- **Descartes verificados:** Rhodes Global SUSPENDIDA por el Board (oficial, jul-2026) · Commonwealth (solo Commonwealth) · Reach Oxford (grado) · GREAT (ARG fuera) · Yale/Harvard (sin full para másters de econ) · AAUW (mujeres) · SI/VLIR/ARES/Holland ya descartadas antes.

### Barrida becas Top 4 UK (24-jul-2026, 5ª tanda) — informe: `BECAS_top4_UK_barrida.md`

4 becas NUEVAS aplicables: **Glasgow GREAT-España £10k** (por pasaporte ESP — distinta de la GREAT España descartada; no apilable con nada, ni Chevening) · **Glasgow Global Leadership £10k ×281** (probable con 1st-class equiv; piso probable de Glasgow sube £5k→£10k) · **Southampton Global Excellence £3k ×83** (auto, first-come por depósito ~15-ene) · **Southampton VC Global Achievers £2k** (auto). Confirmado sin cambios: Bristol (solo Think Big) y Warwick (Regional 2×100% OK; ❗solo rondas de aplicación 1-3; la Europea 1×25% exige domicilio en Europa ❌). GREAT 2026 sin ARG/ESP en Bristol/Warwick/Southampton → re-barrer listas GREAT ~feb-abr 2027. Pendiente: volcar al Excel (hoja Becas UK).

### Hungría / Stipendium Hungaricum — dossier a fondo (2-ago-2026, 7ª tanda) → `HUNGRIA_stipendium_dossier.md`

**Baja de prioridad, con dos hallazgos que reordenan el mapa.**

- **La beca está mal formada para el perfil:** su beneficio central es la matrícula (que el pasaporte ESP ya resuelve barato en media Europa) y el apoyo de vida es **~€210/mes** (HUF 43.700 + 40.000) contra HUF 120-150k de una habitación en Budapest. Es **4-7× peor que DAAD (€992), Aarhus/SU (~€940) o Erasmus Mundus (€1.400)** en la única dimensión que limita. Con dormitorio gratuito cierra ajustado; sin dormitorio no cierra.
- **⚠️ Posible descalificador sin resolver — la ciudadanía española.** El Call oficial solo excluye a dobles ciudadanos húngaros, pero Corvinus publica que SH es *"available for non-EU nationals"*. Conflicto real, mismo patrón que GREAT España e Ireland Fellows. **Un mail lo resuelve: `iara.waisberg@educacion.gob.ar` (autoridad nominadora argentina).**
- **Corrección al supuesto de campo:** NO hay filtro tipo STEMM. El acuerdo con Argentina dice *"Full degree master studies: In any field of interest"*. Economía entra. Cuota argentina: hasta 25 becas/año (fuente argentina).
- **⭐ CEU YA NO ESTÁ EN HUNGRÍA.** Los programas de grado se dictan en **Viena**, acreditados en Austria y EE.UU. **CEU no es alcanzable vía Stipendium Hungaricum.** Pero el **MA in Economics, Data and Policy** (tracks Research pre-PhD / Global Economic Policy / Data Science / Finance; PhD dentro del Vienna Economics Graduate Center con WU y UniWien) es **mejor fit T1 y T2 que cualquier opción húngara**, con ayuda abierta a cualquier nacionalidad: waiver 50-80% + €300-750/mes, y waiver total + €10.000/año para los mejores. Matrícula €12k/año. **→ agregar al radar, mail a `econdept@ceu.edu`.**
- **Contexto país (nuevo):** Orbán perdió en abril-2026; gobierno de Péter Magyar. Hungría **vuelve a Erasmus** desde el próximo año académico y desmonta las fundaciones universitarias (THE, 29-may-2026). Horizon sigue sin resolverse. Elimina la mayor objeción al país, pero deja el **presupuesto del ciclo SH 2027/28 sin confirmar** por el gobierno entrante.
- **Ojo operativo:** la nominación argentina debe llegar a Tempus **antes del 28-feb**; postular por Argentina bilateral Y por SEGIB a la vez es causal de descalificación.
- Pendiente abierto: Corvinus cobra matrícula también a ciudadanos UE (~€4.200/año) y su beca propia de matrícula completa NO cubre Economic Analysis. ELTE y BME quedan sin investigar (tier C).

### Cotejo lista Instagram (1-ago-2026, 6ª tanda) — sección al final de `BECAS_veredictos_lista_24.md`

De ~35 becas del post, 29 ya estaban relevadas. 6 nuevas: **EPFL Excellence Fellowship ⚠️** (CHF 10k/semestre ×4, todas las nacionalidades, se tilda en la app de máster de EPFL; ❗sin MSc Econ — fit = Financial Engineering/MTE, simétrico a ETH ESOP; **1ª ronda 15-dic-2026**, 2ª 31-mar-2027) y 5 descartes verificados: Trudeau ❌ (solo PhD, años 1-2 — guardar para doctorado, temas T2), Hubert Humphrey ❌ (no conducente a título + 5 años de experiencia), RTP Australia ❌ (solo research degrees), SINGA ❌ (PhD, STEM), IsDB ❌ (ARG fuera de ambas listas). **Conclusión: la barrida de becas está saturada — no hay agujeros relevantes.**

### Nórdicos deep dive + reevaluación de 9 becas (17-jul-2026, 4ª tanda)

Informe: **`BECAS_nordicos_y_reevaluaciones.md`**. Síntesis:

- **Nórdicos:** ninguna beca clásica aplica (todas financian a non-EU fee-payers; Manuel ya paga 0 de matrícula). El mecanismo real es el estatus de **estudiante-trabajador EU**: en **Dinamarca, SU ~DKK 7.000+/mes con trabajo formal de 10-12 h/sem** (sostenido 10 semanas antes y durante) → **Aarhus autofinanciable**. Ranking del bloque: Dinamarca > Finlandia (vida barata) > Suecia/SSE (prestigio, caro) > Noruega. Islandia fuera.
- **DAAD Study Scholarship confirmada: deadline 30-oct-2026**, funding oct-2027, título ≤6 años → la pieza que vuelve full el plan Alemania.
- **EPOS con alerta:** exige ≥2 años de experiencia POST-título (verificar contra fecha de egreso); curso estrella T2: HTW Berlin MIDE, **deadline 31-ago-2026**; máx. 3 cursos por ciclo.
- **ESKAS ❌ definitivo para máster** (solo research/PhD desde 2027-28; guardar para el PhD). SISGP y ARES ratificadas ❌ para el ciclo nuevo.
- **Stipendium Hungaricum sube a ⚠️:** Argentina confirmada como sending partner (bilateral + SEGIB), deadline 15-ene-2027, pero requiere **nominación del Ministerio de Educación argentino** — averiguar proceso local.
- **Weidenfeld-Hoffmann reforzada:** el **MSc Economics for Development está en la lista de cursos elegibles** (y Financial Economics p/ T1); exige nacionalidad + residencia en país en desarrollo y compromiso de retorno; statement con template propio, deadline del curso dic-ene.
- **DSU Italia ⚠️ nueva:** need-based regional (hasta ~€7,2k + alojamiento + comedor + waiver, umbral ISEE ~<€26-27k) — condicional al ingreso familiar; calcular ISEE parificato antes de contarla.
- GOI-IES: ciclo 2027/28 TBA, patrón ene-mar-2027.

### Barrida por hueco de cobertura (2-ago-2026, 7ª tanda) — informe: `EXPAND_hallazgos_ago2026.md`

En vez de buscar más becas (saturado), se diffeó **de dónde** salió todo lo guardado: el archivo venía solo de financiadores primarios, del lado argentino de las becas de Estado y de listas/catálogos. Nunca se tocó **el lado español del Estado**, ni institutos/bancos centrales, ni tableros de empleo. Cero entradas de tipo empleador o industria. Hallazgos:

- **⭐⭐ Banco de España — Programa de ayudas para estudios en economía y finanzas ✅ (el hallazgo grande).** 10 plazas. **Matrícula completa aparte + €21.500/año** (Europa/España), £18.500 UK, US$29.000 EEUU; hasta 2 cursos; + estancia de 1-2 meses en el BdE a €1.550/mes. Solo nacionalidad española + **título de grado, sin exigir que sea español** — el filtro que mató a la Caixa acá no está. Centro y titulación a elección. **Cubre UK sin Chevening.** Deadline 2026-27 fue el 18-feb-2026 → esperar bases 2027-28 ~dic-2026, cierre ~feb-2027. ❗**Incompatible con cualquier otra beca simultánea** → reemplaza a Areces/del Pino/Chevening, no se suma; decidir orden de preferencia antes de feb-2027. ❗Exige **2+ informes de profesores enviados directamente por ellos** (los mismos de McCall MacBain → pedirlos de una). ⚠️ Verificar homologación del título UBA con `per.investigacion.becas@bde.es`.
- **⭐ EUI Florencia — cupo nacional español ✅ (para el PhD, no el máster).** 12 becas 2026-27 del Ministerio de Ciencia, €1.650/mes + viaje + seguro; postularse al doctorado *es* postularse a la beca. Exige máster terminado → es el paso POSTERIOR al MSc, pero cambia qué máster conviene elegir. ⚠️ Verificar si el "residencia ≥2 años" aplica también a ciudadanos españoles (`becas.iue@universidades.gob.es`) — mismo patrón que la Caixa/GREAT/Irlanda.
- **Ruta predoc/RA remunerada 🔍 rama nunca abierta.** Es sueldo, no matrícula → no viola la política de "NA solo full funding". Europa: Oxford (GPI/CSAE/EIT), Sciences Po, Zúrich, LSE CEP, Manchester. EEUU: Fed Board + Feds regionales, Yale Tobin, Princeton, Chicago BFI, MIT, SIEPR. Tableros: predoc.org, econjobmarket.org. Ventanas **oct-ene** → se decide en oct-2026.
- **Kiel Institute ASP ⚠️ (post-máster):** 10 meses, sin matrícula, **beca €1.750/mes × 16 meses**, macro/finanzas internacional, PhD-prep. Relaja la tensión "1 año vs. research master de 2 años".
- **Erasmus Mundus: el catálogo nunca se barrió.** 220 proyectos con filtro por campo ECO; solo QEM/EPOG+ están en el archivo. Argentina es país participante y la UBA figura en el catálogo. Acción: filtrar ECO y cotejar contra la hoja "Todos los programas".
- **Contactos:** para MSc taught del continente, mailear profesores tiene ROI bajo (admisión por comité, sin supervisor) — a diferencia de McGill. El esfuerzo de outreach está bien puesto donde está.

## 7. Timeline crítico próximo

> 📅 **Calendario operativo completo: `GANTT_becas_MSc_2027.xml`** (1-ago-2026). Gantt semanal ago-2026 → sep-2027 con las ~45 becas/aplicaciones aplicables, qué documento exige cada deadline, checklist de requisitos por beca, vista inversa de documentos maestros, lista de próximos 90 días y panel de seguimiento. La tabla de abajo es el resumen.

| Fecha | Acción |
|---|---|
| jul-ago 2026 | Preparar GRE (Q≥165/V≥158/AWA≥4.0) |
| ~~4-ago-2026~~ | ~~Abre Chevening~~ ❌ descartada 2-ago-2026 (no llega a 2 años post-título) |
| ~~18-24 jul-2026~~ | ~~Endorsement UBA + referencias McCall MacBain~~ ❌ **desistido 16-ago-2026: el endorsement UBA no salió a tiempo** |
| **31-jul-2026** | Cierra DAAD Helmut Schmidt (solo si interesa PPGG) |
| ~~19-ago-2026~~ | ~~🎯 Deadline McCall MacBain intl~~ ❌ ya no aplica |
| ~ago 2026 | DAAD EPOS |
| sep-oct 2026 | 🎯 Arranca proceso Inova (5 apps UK) — foco inmediato |
| ~dic-2026 | Se publican las bases del **Banco de España** 2027-28 |
| ~18-feb-2027 | 🎯 **Cierre Banco de España** (+ informes de profesores enviados por ellos) |
| ~oct-2026 | Chequear si vuelve Southampton Presidential |
| ~dic 2026 | Tinbergen/ETH deadlines tempranos |
| ~ene 2027 | IELTS rendido |
| ~~15-ene-2027~~ | ~~Deadline McGill MA Economics~~ ❌ ya no aplica |
| ene / feb 2027 | CEMFI funding (31/1) · Areces · del Pino · Carolina (la Caixa ❌, no aplica) |
| ~18-feb-2027 | 🎯 **Cierre Banco de España** — foco inmediato en paralelo con Inova |
| feb-abr 2027 | Aplicaciones rolling UK + continente |
| **1-oct-2026** | 🇨🇭 Abre la ventana de aplicación de HSG St. Gallen (MEcon y MiQE/F) |
| **nov-2026** | 🇨🇭 🎯 **Reabre el portal de HSG Best Talents (cohorte 2027)** — adjudicación rolling, aplicar temprano; exige admisión previa a un máster de HSG |
| 31-ene-2027 | 🇨🇭 Límite para designar programa alternativo (MEcon) en la aplicación de MiQE/F |
| 28-feb-2027 | 🇨🇭 Cierra UNIGE |
| **31-mar-2027** | 🇨🇭 Cierra **HSG MEcon** (GRE opcional) |
| **30-abr-2027** | 🇨🇭 Cierra HSG MiQE/F · **UNIL** (30-abr por ser UE, no 28-feb) · **MIME Basilea** (tardías hasta 31-jul) |
| sep 2027 | 🎓 Inicio |

## 8. Pendientes abiertos clave

1. ~~Verificar la Caixa y Carolina~~ ✔ RESUELTO 17-jul-2026: la Caixa ❌ / Carolina ✅ (bajo nacionalidad ARG).
2. ~~Endorsement UBA para McCall MacBain~~ ❌ CERRADO 16-ago-2026: no salió a tiempo, Manuel desistió de McCall MacBain/McGill. Ver `MCCALL_MCGILL_plan_ataque.md` (queda como registro histórico).
3. ~~Contactar supervisores McGill~~ ❌ CERRADO 16-ago-2026: sin objeto tras desistir de McGill.
4. ⏰ **AHORA (sep-2026): foco triple Banco de España + Inova/UK + HSG St. Gallen** (HSG sumado el 13-sep-2026 por decisión de Manuel). Inova: avanzar documentos y form (arranca sep-oct, `INOVA_todo_documentos.md`). Banco de España: preparar informes de profesores (los mismos que se iban a pedir para McCall MacBain — pedirlos ya) y verificar homologación del título UBA (pendiente 9). HSG: aplicar a MEcon desde el 1-oct-2026 para tener la admisión cuando reabra Best Talents en nov-2026.
5. ⏰ OCT-2026: chequear Southampton Presidential 2027-28 (define slots 4-5 de Inova).
5. Verificar edad ≤25 para IYT Italia al momento de aplicar (conv. abr-may-2027).
6. Resolver tensión 1 año vs. research masters de 2 años (PhD-prep) — decisión de Manuel. McGill MA resuelve parcialmente: 1 año Y PhD-prep.
7. ~~Becas restantes por investigar~~ ✔ CERRADO 17-jul-2026 (ver sección 6). Nuevos pendientes: ~~nominación PBEEE vía McGill~~ ❌ sin objeto (dependía de McGill) · contactar Rotary BA (~sep-2026) · decidir si intake 2028-29 habilita Fulbright · evaluar Western MA en el Ranking.
8. ~~Chevening~~ ✔ CERRADO 2-ago-2026: **descartada por elegibilidad** (§2 bis). Con eso, UK depende del Banco de España o de las becas de universidad.
9. ⏰ **Mail al Banco de España (`per.investigacion.becas@bde.es`), tres preguntas en uno, con las bases 27-28 (~dic-2026).** ✅ **Riesgo acotado el 9-sep-2026:** leídas las [bases oficiales 26-27](https://www.bde.es/f/webbde/PER/convoca/ficheros/descargar/Bases26_27.pdf) completas, **no exigen homologación ni universidad española** — sólo *"nacionalidad española"* + *"título de licenciatura o de grado universitario"*. Deja de ser "lo único que puede tumbar la mejor beca del mapa", pero conviene tenerlo por escrito. Las otras dos preguntas, nuevas y de más impacto: **(b)** ¿un contrato de asistente de investigación en el departamento entra en la excepción de la base 7 (*"ayuda, beca o remuneración... derivadas de actividades de investigación en el área de especialización"*)? — define si el hueco de financiación se puede tapar trabajando; **(c)** ⚠️ ¿el requisito de *"conocer el idioma del país donde vayan a realizar sus estudios"* (base 1) se satisface con inglés en un país donde el idioma oficial es otro? Es un riesgo específico de Suiza, Países Bajos y nórdicos.
10. ⏰ **Preguntar al EUI si el requisito de residencia ≥2 años aplica también a ciudadanos españoles** (`becas.iue@universidades.gob.es`).
11. ⏰ **Verificar DAAD EPOS y World Bank JJ/WBGSP contra la regla de experiencia post-título** (§2 bis). Probablemente caigan igual que Chevening.
12. **Decidir orden de preferencia BdE / Areces / del Pino antes de feb-2027** — el BdE es incompatible con cualquier otra beca simultánea. ✅ **Confirmado 9-sep-2026 contra la base 7: la incompatibilidad alcanza también a las becas propias de universidad**, así que entran en la misma decisión Warwick Regional, Bristol Think Big y **HSG Best Talents**. (Chevening ya no está en la ecuación.)
16. ⏰ **Suiza (9-sep-2026, ver `SUIZA_dossier.md` §7):** mail a HSG por el monto del aporte de vida de Best Talents y su compatibilidad con beca externa · mail a `HECmasterAdmission@unil.ch` por el riesgo de *bridging programme* con título UBA (**UNIL no se puede rankear hasta saberlo**) · conversión de la nota UBA a la escala suiza /6 (define si MIME exige GRE) · deadline de aplicación de UZH ⚠️ sin verificar · vía de exención del KVG (~CHF 4.000/año).
17. ⛔ **Cargar en `masters_consolidado_GRE_2027.xlsx` (hoja "Todos los programas") los 9 programas suizos** de `SUIZA_dossier.md` §1: HSG MEcon y MiQE/F, UZH, UNIL, MIME Basilea, IHEID, UNIGE, UniBE, UniNE. Mismo bloqueo de scripting que los puntos 13 y 14.
13. Filtrar el catálogo Erasmus Mundus por campo ECO y cotejar contra la hoja "Todos los programas".
13. ⛔ **Pendiente manual:** `masters_consolidado_GRE_2027.xlsx` y `tabla_becas_uk.xlsx` no pudieron editarse por script (el entorno de scripting no arrancó). Las filas están listas para copiar y pegar en **`ACTUALIZACION_excels_7a_tanda.xml`** (4 hojas: Becas nuevas · Timeline correcciones · Pendientes nuevos · UK). ❗Antes de pegar hay que **borrar la fila vieja de Chevening** ("7-oct-2026 abre") de la hoja Timeline.
14. ⏰ (25-ago-2026) **Agregar a `masters_consolidado_GRE_2027.xlsx` (hoja "Todos los programas") los 5 candidatos alemanes nuevos** del dossier DAAD: HU Berlin MSc Economics, Heidelberg AWI, Kiel (Quant. Economics/Economics), Cologne Economic Research, Konstanz/Tübingen (con reservas) — ver `DAAD_STUDY_SCHOLARSHIP_dossier.md` §2.2. Mismo bloqueo de scripting que el punto 13.
15. ⏰ Confirmar fecha exacta del deadline DAAD Study Scholarship ciclo 2026 (funding oct-2027) en el portal DAAD seleccionando Argentina como país de origen — el dato "~31-oct-2026" es estimado por precedente, no confirmado. Mail opcional de buena práctica a `info.buenosaires@daad.de`.
